...
while (my $line = <IN>){
	...
	next if (length $protein_name == 0);#it could happen e.g. decoy hit in MS-GF+. Because no protei id, no protein sequence, so jump to next
...
sub readFasta(){
	...
	while(my $line=<IN>){
		...
		if($line=~/^>/){
			unless(length $seq==0){
				#the header can not be directly used
				#In the PIT pipeline, one ORF (protein) can be from multiple transcripts which is separated by ,
				...
				for (my $i=0;$i<scalar @tmp;$i++){
					...
					#print "current id: <$id>\n";
					$seqs{$id}=$seq;
					#only keep the accession before first space
					my ($first) = split(" ",$id);
					$seqs{$first} = $seq;
				}
				$seq="";
